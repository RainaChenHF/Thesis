"""
H-FAct v3: Hybrid-FActScore with modality routing, answer-relevance gating,
enriched decomposition, and 4-level graded fact verification.

Evolution from v1 (Stage-2 binary) to v3 (Stage-3 headline metric):
  v1: Binary supported/not-supported verification (98.2% scores in {0,1}).
  v2: + relevance gate + enriched decomposition. Still binary by default.
  v3: + 4-level graded verification (FULLY/MOSTLY/WEAKLY/NOT supported).

Usage (v3 is the headline metric and the default behavior):
  from src.metrics.hybrid_eval_v3 import hybrid_factscore_v3
  result = hybrid_factscore_v3(question, answer, table_ctx, text_ctx)

Legacy v2 binary mode is available via:
  result = hybrid_factscore_v3(..., graded=False)
  # or via back-compat alias: hybrid_factscore_v2(...)
"""
...
# ── H-FAct v3 Main Function ──────────────────────────────────────────────

def hybrid_factscore_v3(
    question: str, answer: str,
    table_ctx: str, text_ctx: str,
    use_relevance_gate: bool = True,
    graded: bool = True,   # v3 default: 4-level graded verification
) -> dict:
    """
    H-FAct v3: relevance gate + enriched decomposition + (default) graded verification.
    ...
    result["h_fact_v3"] = round(score, 4)
...
# Legacy v2 consumers (and code that hasn't been updated to v3 names) can still
# call hybrid_factscore_v2; it delegates to v3 with the legacy default graded=False.

def hybrid_factscore_v2(
    question: str, answer: str,
    table_ctx: str, text_ctx: str,
    use_relevance_gate: bool = True,
    graded: bool = False,   # legacy v2 default: binary verification
) -> dict:
    """Legacy v2 entry point. Delegates to v3 with binary-mode default for backward compatibility."""
    result = hybrid_factscore_v3(
        question, answer, table_ctx, text_ctx,
        use_relevance_gate=use_relevance_gate, graded=graded,
    )
    # Mirror legacy key name for any consumer that reads result["h_fact_v2"]
    result["h_fact_v2"] = result.get("h_fact_v3", 0.0)
    return result
